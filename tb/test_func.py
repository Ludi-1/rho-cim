...
class CIM_Tile:
    def __init__(self, dut, row, col):
        outbuf_datatype_size: int = (
            dut.out_buf_datatype_size.value.integer
        ...
        self.outbuf = [
            LogicArray(0, Range(outbuf_datatype_size - 1, "downto", 0))
        ] * int(tile_columns / max_datatype_size)
    ...
    async def output_buf_co(self, dut):
        ...
        while True:
            ...
            o_obuf_addr = (
                dut.o_addr_out_buf.value.integer
            )  # Output buffer address count
            self.output_data = LogicArray(
                self.outbuf[o_obuf_addr], Range(outbuf_datatype_size - 1, "downto", 0)
    ...
    async def i_data_update(self, dut):
        while True:
            ...
            for row in range(0, self.row):
                for col in range(0, self.col):
                    i_data_vector += (
                        self.tiles[row][col].outbuf[o_obuf_addr].binstr
                    )  # Value from simulated obuf
...
async def func_test_1(dut):
    ...
    print(
        dut.row_split_tiles.value.integer,
        dut.col_split_tiles.value.integer,
        dut.func_datatype_size.value.integer,
    )
